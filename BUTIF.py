...
class Clustering(BaseEstimator):
    ...
    def fit(self, X):
        ...
        self.clusters_ = np.array(self.clusters_)
            ...
            if len(L) == 1:
                ...
                for i in L.keys():
                    for j in L.keys():
                        ...
                        distance = norm(L[i].centroid - L[i].centroid)
                ...
                objects_ = np.vstack((left_child.objects, 
                                    right_child.objects))
                labels_ = np.ones(objects_.shape[0])
                labels_[: left_child.objects.shape[0]] = -1.0
                labels_[left_child.objects.shape[0]: ] = 1.0
                
                indices_ = np.arange(self.n_features)
                if self.selector is not None:
                    self.selector.fit(objects_, labels_)
                    indices_ = self.selector.scores_.argsort()[: self._k_features]
                ...
                new_node = Node(objects_, labels_, 
        ...
        if self.task == 'classification':
            ...
            for c in self.classes_:
                ...
                for cluster in partition:
                    leaf = Node(objects_in_c[cluster], c * np.ones(len(cluster)), is_leaf=True)
                    leaf.class_ = c
            ...
            for i, cluster in enumerate(partition):
                leaf = Node(X[cluster], i * np.ones(len(cluster)), is_leaf=True)
                leaf.class_ = np.mean(y[cluster])
                self._leaves[str(leaf_cnt)] = leaf
                leaf_cnt += 1
